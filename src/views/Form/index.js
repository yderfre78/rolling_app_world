...
export default function FormDeparture() {
  ...
  const { getFlightFull, isLoading } = useContext(FlightContext);
  ...
  const [failureReturnDate, setFailureReturnDate] = useState(false);
  ...
  const HandleSubmit = (e, valores) => {
    ...
    if (returnDate <= departureDate) {
      setFailureReturnDate(true);
      setFailureMessageReturnDate(
        "Fecha de Retorno no puede ser menor a la de Salida"
      );
      setTimeout(() => setFailureReturnDate(false), 5000);
      setTimeout(() => setFailureMessageReturnDate(false), 5000);
      return;
    }

    if (!returnDate.trim()) {
      setFailureReturnDate(true);
      setFailureMessageReturnDate("Fecha de Regreso es obligatorio");
      setTimeout(() => setFailureReturnDate(false), 5000);
      setTimeout(() => setFailureMessageReturnDate(false), 5000);
      return;
    }
                      ...
                      <Form onSubmit={HandleSubmit} className="formulario">
                        ...
                        <div className="container">
                          <div className="row">
                            <div className="col">
                              <TextField
                                helperText={failureMessageReturnDate}
                                error={failureReturnDate}
                                sx={{ width: 300 }}
                                id="returnDate"
                                label="Retorno"
                                onChange={(ev) =>
